#!/usr/bin/env python3
"""
Warmly Telegram Bot - Working Version
Рабочая версия бота для тестирования
"""
...
def button_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "mood":
        show_mood_menu(query, context)
    elif query.data == "motivation":
        show_motivation(query, context)
    elif query.data == "archive":
        show_archive(query, context)
    elif query.data == "stats":
        show_stats(query, context)
    elif query.data.startswith("mood_"):
        mood = query.data.split("_")[1]
        handle_mood_selection(query, context, mood)
    elif query.data == "back_to_main":
        show_main_menu(query, context)
    elif query.data.startswith("save_"):
        handle_save_callback(query, context)

def show_mood_menu(query, context):
...
def handle_mood_selection(query, context, mood):
...
def show_motivation(query, context):
    ...
    query.edit_message_text(f"💝 {phrase}", reply_markup=reply_markup)

def show_archive(query, context):
...
def handle_save_callback(query, context):
    ...
    if data.startswith("save_phrase_"):
        ...
        query.answer("💝 Сохранено в архив!")
...
def main():
    ...
    dispatcher.add_handler(CallbackQueryHandler(handle_save_callback, pattern="^save_"))
    
    # Запускаем бота
    logger.info("🚀 Запуск Working Warmly Bot...")
